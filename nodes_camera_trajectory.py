...
CAMERA_DICT = {'base_T_norm': 1.5, 'base_angle': np.pi / 3, 'Static': {'angle': [0.0, 0.0, 0.0], 'T': [0.0, 0.0, 0.0]}, 'Pan Up': {'angle': [0.0, 0.0, 0.0], 'T': [0.0, -1.0, 0.0]}, 'Pan Down': {'angle': [0.0, 0.0, 0.0], 'T': [0.0, 1.0, 0.0]}, 'Pan Left': {'angle': [0.0, 0.0, 0.0], 'T': [-1.0, 0.0, 0.0]}, 'Pan Right': {'angle': [0.0, 0.0, 0.0], 'T': [1.0, 0.0, 0.0]}, 'Zoom In': {'angle': [0.0, 0.0, 0.0], 'T': [0.0, 0.0, 2.0]}, 'Zoom Out': {'angle': [0.0, 0.0, 0.0], 'T': [0.0, 0.0, -2.0]}, 'Anti Clockwise (ACW)': {'angle': [0.0, 0.0, -1.0], 'T': [0.0, 0.0, 0.0]}, 'ClockWise (CW)': {'angle': [0.0, 0.0, 1.0], 'T': [0.0, 0.0, 0.0]}}
...
def ray_condition(K, c2w, H, W, device):
    ...
    """
    B = K.shape[0]
    j, i = torch.meshgrid(torch.linspace(0, H - 1, H, device=device, dtype=c2w.dtype), torch.linspace(0, W - 1, W, device=device, dtype=c2w.dtype), indexing='ij')
...
class WanCameraEmbedding_ISO(io.ComfyNode):
    ...
    def execute(cls, camera_pose, width, height, length, speed=1.0, fx=0.5, fy=0.5, cx=0.5, cy=0.5) -> io.NodeOutput:
        ...
        control_camera_video = torch.concat([torch.repeat_interleave(control_camera_video[:, :, 0:1], repeats=4, dim=2), control_camera_video[:, :, 1:]], dim=2).transpose(1, 2)
...
class CameraTrajectoryExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [WanCameraEmbedding_ISO]

async def comfy_entrypoint() -> CameraTrajectoryExtension_ISO:
    return CameraTrajectoryExtension_ISO()