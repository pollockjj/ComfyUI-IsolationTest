...
class EmptyCosmosLatentVideo_ISO(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(node_id='EmptyCosmosLatentVideo_ISO', category='latent/video', inputs=[io.Int.Input('width', default=1280, min=16, max=nodes.MAX_RESOLUTION, step=16), io.Int.Input('height', default=704, min=16, max=nodes.MAX_RESOLUTION, step=16), io.Int.Input('length', default=121, min=1, max=nodes.MAX_RESOLUTION, step=8), io.Int.Input('batch_size', default=1, min=1, max=4096)], outputs=[io.Latent.Output()])
...
class CosmosExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [EmptyCosmosLatentVideo_ISO, CosmosImageToVideoLatent_ISO, CosmosPredict2ImageToVideoLatent_ISO]

async def comfy_entrypoint() -> CosmosExtension_ISO:
    return CosmosExtension_ISO()