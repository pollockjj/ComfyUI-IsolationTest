...
def camera_embeddings(elevation, azimuth):
    ...
    embeddings = torch.stack([torch.deg2rad(90 - elevation - 90), torch.sin(torch.deg2rad(azimuth)), torch.cos(torch.deg2rad(azimuth)), torch.deg2rad(90 - torch.full_like(elevation, 0))], dim=-1).unsqueeze(1)
    return embeddings
...
class SV3D_Conditioning_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SV3D_Conditioning_ISO', category='conditioning/3d_models', inputs=[io.ClipVision.Input('clip_vision'), io.Image.Input('init_image'), io.Vae.Input('vae'), io.Int.Input('width', default=576, min=16, max=nodes.MAX_RESOLUTION, step=8), io.Int.Input('height', default=576, min=16, max=nodes.MAX_RESOLUTION, step=8), io.Int.Input('video_frames', default=21, min=1, max=4096), io.Float.Input('elevation', default=0.0, min=-90.0, max=90.0, step=0.1, round=False)], outputs=[io.Conditioning.Output(display_name='positive'), io.Conditioning.Output(display_name='negative'), io.Latent.Output(display_name='latent')])
...
class Stable3DExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [StableZero123_Conditioning_ISO, StableZero123_Conditioning_Batched_ISO, SV3D_Conditioning_ISO]

async def comfy_entrypoint() -> Stable3DExtension_ISO:
    return Stable3DExtension_ISO()