...
class TomePatchModel_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='TomePatchModel_ISO', category='model_patches/unet', inputs=[io.Model.Input('model'), io.Float.Input('ratio', default=0.3, min=0.0, max=1.0, step=0.01)], outputs=[io.Model.Output()])
...
class TomePatchModelExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [TomePatchModel_ISO]

async def comfy_entrypoint() -> TomePatchModelExtension_ISO:
    return TomePatchModelExtension_ISO()