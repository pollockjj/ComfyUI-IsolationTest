...
class LoraSave_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='LoraSave_ISO', display_name='Extract and Save Lora_ISO', category='_for_testing', inputs=[io.String.Input('filename_prefix', default='loras/ComfyUI_extracted_lora'), io.Int.Input('rank', default=8, min=1, max=4096, step=1), io.Combo.Input('lora_type', options=tuple(LORA_TYPES.keys())), io.Boolean.Input('bias_diff', default=True), io.Model.Input('model_diff', tooltip='The ModelSubtract output to be converted to a lora.', optional=True), io.Clip.Input('text_encoder_diff', tooltip='The CLIPSubtract output to be converted to a lora.', optional=True)], is_experimental=True, is_output_node=True)
...
class LoraSaveExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [LoraSave_ISO]

async def comfy_entrypoint() -> LoraSaveExtension_ISO:
    return LoraSaveExtension_ISO()