...
class EpsilonScaling_ISO(io.ComfyNode):
    """
    ...
    def define_schema(cls):
        return io.Schema(node_id='Epsilon Scaling_ISO', category='model_patches/unet', inputs=[io.Model.Input('model'), io.Float.Input('scaling_factor', default=1.005, min=0.5, max=1.5, step=0.001, display_mode=io.NumberDisplay.number)], outputs=[io.Model.Output()])

    @classmethod
    def execute(cls, model, scaling_factor) -> io.NodeOutput:
        if scaling_factor == 0:
            scaling_factor = 1e-09
...
class EpsilonScalingExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [EpsilonScaling_ISO, TemporalScoreRescaling_ISO]

async def comfy_entrypoint() -> EpsilonScalingExtension_ISO:
    return EpsilonScalingExtension_ISO()