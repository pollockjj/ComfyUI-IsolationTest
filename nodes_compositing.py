...
def porter_duff_composite(src_image: torch.Tensor, src_alpha: torch.Tensor, dst_image: torch.Tensor, dst_alpha: torch.Tensor, mode: PorterDuffMode):
    ...
    if mode == PorterDuffMode.ADD:
        ...
        return (None, None)
    out_image = torch.where(out_alpha > 1e-05, out_image / out_alpha, torch.zeros_like(out_image))
...
class PorterDuffImageComposite_ISO(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(node_id='PorterDuffImageComposite_ISO', display_name='Porter-Duff Image Composite_ISO', category='mask/compositing', inputs=[io.Image.Input('source'), io.Mask.Input('source_alpha'), io.Image.Input('destination'), io.Mask.Input('destination_alpha'), io.Combo.Input('mode', options=[mode.name for mode in PorterDuffMode], default=PorterDuffMode.DST.name)], outputs=[io.Image.Output(), io.Mask.Output()])
...
class SplitImageWithAlpha_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SplitImageWithAlpha_ISO', display_name='Split Image with Alpha_ISO', category='mask/compositing', inputs=[io.Image.Input('image')], outputs=[io.Image.Output(), io.Mask.Output()])
...
class JoinImageWithAlpha_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='JoinImageWithAlpha_ISO', display_name='Join Image with Alpha_ISO', category='mask/compositing', inputs=[io.Image.Input('image'), io.Mask.Input('alpha')], outputs=[io.Image.Output()])
...
class CompositingExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [PorterDuffImageComposite_ISO, SplitImageWithAlpha_ISO, JoinImageWithAlpha_ISO]

async def comfy_entrypoint() -> CompositingExtension_ISO:
    return CompositingExtension_ISO()