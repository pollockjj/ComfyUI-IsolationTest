...
class APG_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(node_id='APG_ISO', display_name='Adaptive Projected Guidance_ISO', category='sampling/custom_sampling', inputs=[io.Model.Input('model'), io.Float.Input('eta', default=1.0, min=-10.0, max=10.0, step=0.01, tooltip='Controls the scale of the parallel guidance vector. Default CFG behavior at a setting of 1.'), io.Float.Input('norm_threshold', default=5.0, min=0.0, max=50.0, step=0.1, tooltip='Normalize guidance vector to this value, normalization disable at a setting of 0.'), io.Float.Input('momentum', default=0.0, min=-5.0, max=1.0, step=0.01, tooltip='Controls a running average of guidance during diffusion, disabled at a setting of 0.')], outputs=[io.Model.Output()])
...
class ApgExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [APG_ISO]

async def comfy_entrypoint() -> ApgExtension_ISO:
    return ApgExtension_ISO()