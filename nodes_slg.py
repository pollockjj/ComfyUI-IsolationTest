...
class SkipLayerGuidanceDiT_ISO(io.ComfyNode):
    """
    ...
    def define_schema(cls):
        return io.Schema(node_id='SkipLayerGuidanceDiT_ISO', category='advanced/guidance', description='Generic version of SkipLayerGuidance node that can be used on every DiT model.', is_experimental=True, inputs=[io.Model.Input('model'), io.String.Input('double_layers', default='7, 8, 9'), io.String.Input('single_layers', default='7, 8, 9'), io.Float.Input('scale', default=3.0, min=0.0, max=10.0, step=0.1), io.Float.Input('start_percent', default=0.01, min=0.0, max=1.0, step=0.001), io.Float.Input('end_percent', default=0.15, min=0.0, max=1.0, step=0.001), io.Float.Input('rescaling_scale', default=0.0, min=0.0, max=10.0, step=0.01)], outputs=[io.Model.Output()])
    ...
    def execute(cls, model, scale, start_percent, end_percent, double_layers='', single_layers='', rescaling_scale=0) -> io.NodeOutput:
        ...
        double_layers = re.findall('\\d+', double_layers)
        double_layers = [int(i) for i in double_layers]
        single_layers = re.findall('\\d+', single_layers)
    ...
    skip_guidance = execute

class SkipLayerGuidanceDiTSimple_ISO(io.ComfyNode):
    """
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SkipLayerGuidanceDiTSimple_ISO', category='advanced/guidance', description='Simple version of the SkipLayerGuidanceDiT node that only modifies the uncond pass.', is_experimental=True, inputs=[io.Model.Input('model'), io.String.Input('double_layers', default='7, 8, 9'), io.String.Input('single_layers', default='7, 8, 9'), io.Float.Input('start_percent', default=0.0, min=0.0, max=1.0, step=0.001), io.Float.Input('end_percent', default=1.0, min=0.0, max=1.0, step=0.001)], outputs=[io.Model.Output()])
...
class SkipLayerGuidanceExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [SkipLayerGuidanceDiT_ISO, SkipLayerGuidanceDiTSimple_ISO]

async def comfy_entrypoint() -> SkipLayerGuidanceExtension_ISO:
    return SkipLayerGuidanceExtension_ISO()