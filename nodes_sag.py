...
class SelfAttentionGuidance_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SelfAttentionGuidance_ISO', display_name='Self-Attention Guidance_ISO', category='_for_testing', inputs=[io.Model.Input('model'), io.Float.Input('scale', default=0.5, min=-2.0, max=5.0, step=0.01), io.Float.Input('blur_sigma', default=2.0, min=0.0, max=10.0, step=0.1)], outputs=[io.Model.Output()], is_experimental=True)
...
class SagExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [SelfAttentionGuidance_ISO]

async def comfy_entrypoint() -> SagExtension_ISO:
    return SagExtension_ISO()