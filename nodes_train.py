...
class TrainSampler(comfy.samplers.Sampler):
    ...
    def fwd_bwd(self, model_wrap, batch_sigmas, batch_noise, batch_latent, cond, indicies, extra_args, dataset_size, bwd=True):
        xt = model_wrap.inner_model.model_sampling.noise_scaling(batch_sigmas, batch_noise, batch_latent, False)
        x0 = model_wrap.inner_model.model_sampling.noise_scaling(torch.zeros_like(batch_sigmas), torch.zeros_like(batch_noise), batch_latent, False)
        ...
        with torch.autocast(xt.device.type, dtype=self.training_dtype):
            x0_pred = model_wrap(xt.requires_grad_(True), batch_sigmas.requires_grad_(True), **batch_extra_args)
    ...
    def sample(self, model_wrap, sigmas, extra_args, callback, noise, latent_image=None, denoise_mask=None, disable_pbar=False):
        ...
        for i in (pbar := trange(self.total_steps, desc='Training LoRA', smoothing=0.01, disable=not comfy.utils.PROGRESS_BAR_ENABLED)):
            ...
            if self.real_dataset is None:
                ...
                batch_sigmas = [model_wrap.inner_model.model_sampling.percent_to_sigma(torch.rand((1,)).item()) for _ in range(min(self.batch_size, dataset_size))]
                batch_sigmas = torch.tensor(batch_sigmas).to(batch_latent.device)
                loss = self.fwd_bwd(model_wrap, batch_sigmas, batch_noise, batch_latent, cond, indicies, extra_args, dataset_size, bwd=True)
                ...
                for index in indicies:
                    ...
                    batch_sigmas = model_wrap.inner_model.model_sampling.percent_to_sigma(torch.rand((1,)).item())
                    batch_sigmas = torch.tensor([batch_sigmas]).to(single_latent.device)
                    loss = self.fwd_bwd(model_wrap, batch_sigmas, batch_noise, single_latent, cond, [index], extra_args, dataset_size, bwd=False)
                    total_loss += loss
...
class TrainLoraNode_ISO(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(node_id='TrainLoraNode_ISO', display_name='Train LoRA_ISO', category='training', is_experimental=True, is_input_list=True, inputs=[io.Model.Input('model', tooltip='The model to train the LoRA on.'), io.Latent.Input('latents', tooltip='The Latents to use for training, serve as dataset/input of the model.'), io.Conditioning.Input('positive', tooltip='The positive conditioning to use for training.'), io.Int.Input('batch_size', default=1, min=1, max=10000, tooltip='The batch size to use for training.'), io.Int.Input('grad_accumulation_steps', default=1, min=1, max=1024, tooltip='The number of gradient accumulation steps to use for training.'), io.Int.Input('steps', default=16, min=1, max=100000, tooltip='The number of steps to train the LoRA for.'), io.Float.Input('learning_rate', default=0.0005, min=1e-07, max=1.0, step=1e-07, tooltip='The learning rate to use for training.'), io.Int.Input('rank', default=8, min=1, max=128, tooltip='The rank of the LoRA layers.'), io.Combo.Input('optimizer', options=['AdamW', 'Adam', 'SGD', 'RMSprop'], default='AdamW', tooltip='The optimizer to use for training.'), io.Combo.Input('loss_function', options=['MSE', 'L1', 'Huber', 'SmoothL1'], default='MSE', tooltip='The loss function to use for training.'), io.Int.Input('seed', default=0, min=0, max=18446744073709551615, tooltip='The seed to use for training (used in generator for LoRA weight initialization and noise sampling)'), io.Combo.Input('training_dtype', options=['bf16', 'fp32'], default='bf16', tooltip='The dtype to use for training.'), io.Combo.Input('lora_dtype', options=['bf16', 'fp32'], default='bf16', tooltip='The dtype to use for lora.'), io.Combo.Input('algorithm', options=list(adapter_maps.keys()), default=list(adapter_maps.keys())[0], tooltip='The algorithm to use for training.'), io.Boolean.Input('gradient_checkpointing', default=True, tooltip='Use gradient checkpointing for training.'), io.Combo.Input('existing_lora', options=folder_paths.get_filename_list('loras') + ['[None]'], default='[None]', tooltip='The existing LoRA to append to. Set to None for new LoRA.')], outputs=[io.Model.Output(display_name='model', tooltip='Model with LoRA applied'), io.Custom('LORA_MODEL').Output(display_name='lora', tooltip='LoRA weights'), io.Custom('LOSS_MAP').Output(display_name='loss_map', tooltip='Loss history'), io.Int.Output(display_name='steps', tooltip='Total training steps')])

    @classmethod
    def execute(cls, model, latents, positive, batch_size, steps, grad_accumulation_steps, learning_rate, rank, optimizer, loss_function, seed, training_dtype, lora_dtype, algorithm, gradient_checkpointing, existing_lora):
        model = model[0]
        ...
        if len(positive) == 1 and num_images > 1:
            ...
            raise ValueError(f'Number of positive conditions ({len(positive)}) does not match number of images ({num_images}).')
        with torch.inference_mode(False):
            ...
            for n, m in mp.model.named_modules():
                if hasattr(m, 'weight_function'):
                    if m.weight is not None:
                        ...
                        if len(shape) >= 2:
                            ...
                            for adapter_cls in adapters:
                                existing_adapter = adapter_cls.load(n, existing_weights, alpha, dora_scale)
                            ...
                            if existing_adapter is not None:
                                ...
                                train_adapter = adapter_cls.create_train(m.weight, rank=rank, alpha=1.0).to(lora_dtype)
                            for name, parameter in train_adapter.named_parameters():
                                lora_sd[f'{n}.{name}'] = parameter
                            mp.add_weight_wrapper(key, train_adapter)
                            all_weight_adapters.append(train_adapter)
                        else:
                            diff = torch.nn.Parameter(torch.zeros(m.weight.shape, dtype=lora_dtype, requires_grad=True))
                    ...
                    if hasattr(m, 'bias') and m.bias is not None:
                        key = '{}.bias'.format(n)
                        bias = torch.nn.Parameter(torch.zeros(m.bias.shape, dtype=lora_dtype, requires_grad=True))
            ...
            comfy.model_management.load_models_gpu([mp], memory_required=1e+20, force_full_load=True)
            loss_map = {'loss': []}

            def loss_callback(loss):
                loss_map['loss'].append(loss)
            train_sampler = TrainSampler(criterion, optimizer, loss_callback=loss_callback, batch_size=batch_size, grad_acc=grad_accumulation_steps, total_steps=steps * grad_accumulation_steps, seed=seed, training_dtype=dtype, real_dataset=latents if multi_res else None)
            ...
            try:
                ...
                guider.sample(noise.generate_noise({'samples': latents}), latents, train_sampler, sigmas, seed=noise.seed)
            finally:
...
class LossGraphNode_ISO(io.ComfyNode):
    ...
    def execute(cls, loss, filename_prefix, prompt=None, extra_pnginfo=None):
        ...
        draw.text((margin - 30, height - 10), f'{min_loss:.2f}', font=font, fill='black')
        img_array = np.array(img).astype(np.float32) / 255.0
        img_tensor = torch.from_numpy(img_array)[None,]
        return io.NodeOutput(ui=ui.PreviewImage(img_tensor, cls=cls))

class TrainingExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [TrainLoraNode_ISO, LoraModelLoader_ISO, SaveLoRA_ISO, LossGraphNode_ISO]

async def comfy_entrypoint() -> TrainingExtension_ISO:
    return TrainingExtension_ISO()