...
class PhotoMakerLoader_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='PhotoMakerLoader_ISO', category='_for_testing/photomaker', inputs=[io.Combo.Input('photomaker_model_name', options=folder_paths.get_filename_list('photomaker'))], outputs=[io.Photomaker.Output()], is_experimental=True)
...
class PhotoMakerEncode_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='PhotoMakerEncode_ISO', category='_for_testing/photomaker', inputs=[io.Photomaker.Input('photomaker'), io.Image.Input('image'), io.Clip.Input('clip'), io.String.Input('text', multiline=True, dynamic_prompts=True, default='photograph of photomaker')], outputs=[io.Conditioning.Output()], is_experimental=True)
...
class PhotomakerExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [PhotoMakerLoader_ISO, PhotoMakerEncode_ISO]

async def comfy_entrypoint() -> PhotomakerExtension_ISO:
    return PhotomakerExtension_ISO()