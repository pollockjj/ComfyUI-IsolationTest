...
class SaveVideo_ISO(io.ComfyNode):
    ...
    def execute(cls, video: VideoInput, filename_prefix, format: str, codec) -> io.NodeOutput:
        ...
        video.save_to(os.path.join(full_output_folder, file), format=VideoContainer(format), codec=codec, metadata=saved_metadata)
        return io.NodeOutput(ui=ui.PreviewVideo([ui.SavedResult(file, subfolder, io.FolderType.output)]))
...
class CreateVideo_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='CreateVideo_ISO', display_name='Create Video_ISO', category='image/video', description='Create a video from images.', inputs=[io.Image.Input('images', tooltip='The images to create a video from.'), io.Float.Input('fps', default=30.0, min=1.0, max=120.0, step=1.0), io.Audio.Input('audio', optional=True, tooltip='The audio to add to the video.')], outputs=[io.Video.Output()])

    @classmethod
    def execute(cls, images: ImageInput, fps: float, audio: Optional[AudioInput]=None) -> io.NodeOutput:
        return io.NodeOutput(VideoFromComponents(VideoComponents(images=images, audio=audio, frame_rate=Fraction(fps))))

class GetVideoComponents_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='GetVideoComponents_ISO', display_name='Get Video Components_ISO', category='image/video', description='Extracts all components from a video: frames, audio, and framerate.', inputs=[io.Video.Input('video', tooltip='The video to extract components from.')], outputs=[io.Image.Output(display_name='images'), io.Audio.Output(display_name='audio'), io.Float.Output(display_name='fps')])
...
class LoadVideo_ISO(io.ComfyNode):
    ...
    def define_schema(cls):
        ...
        return io.Schema(node_id='LoadVideo_ISO', display_name='Load Video_ISO', category='image/video', inputs=[io.Combo.Input('file', options=sorted(files), upload=io.UploadType.video)], outputs=[io.Video.Output()])
...
class VideoExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [SaveWEBM_ISO, SaveVideo_ISO, CreateVideo_ISO, GetVideoComponents_ISO, LoadVideo_ISO]

async def comfy_entrypoint() -> VideoExtension_ISO:
    return VideoExtension_ISO()