...
class EmptyChromaRadianceLatentImage_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(node_id='EmptyChromaRadianceLatentImage_ISO', category='latent/chroma_radiance', inputs=[io.Int.Input(id='width', default=1024, min=16, max=nodes.MAX_RESOLUTION, step=16), io.Int.Input(id='height', default=1024, min=16, max=nodes.MAX_RESOLUTION, step=16), io.Int.Input(id='batch_size', default=1, min=1, max=4096)], outputs=[io.Latent().Output()])
...
class ChromaRadianceExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [EmptyChromaRadianceLatentImage_ISO, ChromaRadianceOptions_ISO]

async def comfy_entrypoint() -> ChromaRadianceExtension_ISO:
    return ChromaRadianceExtension_ISO()