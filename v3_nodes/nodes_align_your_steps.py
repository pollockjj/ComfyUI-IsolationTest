...
_orig = importlib.import_module('comfy_extras.nodes_align_your_steps')

class AlignYourStepsScheduler_ISO(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        orig_cls = getattr(_orig, 'AlignYourStepsScheduler')
        orig_schema = orig_cls.define_schema()
        schema = copy.deepcopy(orig_schema)
        base_node_id = schema.node_id or 'AlignYourStepsScheduler'
        base_display = schema.display_name or 'AlignYourStepsScheduler'
...
__all__ = ['AlignYourStepsScheduler_ISO']