...
_orig = importlib.import_module('comfy_extras.nodes_ip2p')

class InstructPixToPixConditioning_ISO(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        orig_cls = getattr(_orig, 'InstructPixToPixConditioning')
        orig_schema = orig_cls.define_schema()
        schema = copy.deepcopy(orig_schema)
        base_node_id = schema.node_id or 'InstructPixToPixConditioning'
        base_display = schema.display_name or 'InstructPixToPixConditioning'
...
__all__ = ['InstructPixToPixConditioning_ISO']