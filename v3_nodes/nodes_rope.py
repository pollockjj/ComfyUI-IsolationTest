...
_orig = importlib.import_module('comfy_extras.nodes_rope')

class ScaleROPE_ISO(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        orig_cls = getattr(_orig, 'ScaleROPE')
        orig_schema = orig_cls.define_schema()
        schema = copy.deepcopy(orig_schema)
        base_node_id = schema.node_id or 'ScaleROPE'
        base_display = schema.display_name or 'ScaleROPE'
