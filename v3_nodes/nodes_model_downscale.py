...
_orig = importlib.import_module('comfy_extras.nodes_model_downscale')

class PatchModelAddDownscale_ISO(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        orig_cls = getattr(_orig, 'PatchModelAddDownscale')
        orig_schema = orig_cls.define_schema()
        schema = copy.deepcopy(orig_schema)
        base_node_id = schema.node_id or 'PatchModelAddDownscale'
        base_display = schema.display_name or 'PatchModelAddDownscale (Kohya Deep Shrink)'
...
__all__ = ['PatchModelAddDownscale_ISO']