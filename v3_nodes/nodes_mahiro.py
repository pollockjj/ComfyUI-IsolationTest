...
_orig = importlib.import_module('comfy_extras.nodes_mahiro')

class Mahiro_ISO(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        orig_cls = getattr(_orig, 'Mahiro')
        orig_schema = orig_cls.define_schema()
        schema = copy.deepcopy(orig_schema)
        base_node_id = schema.node_id or 'Mahiro'
        base_display = schema.display_name or 'Mahiro is so cute that she deserves a better guidance function!! (。・ω・。)'
