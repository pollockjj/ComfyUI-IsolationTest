...
_orig = importlib.import_module('comfy_extras.nodes_sdupscale')

class SD_4XUpscale_Conditioning_ISO(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        orig_cls = getattr(_orig, 'SD_4XUpscale_Conditioning')
        orig_schema = orig_cls.define_schema()
        schema = copy.deepcopy(orig_schema)
        base_node_id = schema.node_id or 'SD_4XUpscale_Conditioning'
        base_display = schema.display_name or 'SD_4XUpscale_Conditioning'
...
__all__ = ['SD_4XUpscale_Conditioning_ISO']