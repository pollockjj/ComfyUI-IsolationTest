...
_orig = importlib.import_module('comfy_extras.nodes_edit_model')

class ReferenceLatent_ISO(io.ComfyNode):
    @classmethod
    def define_schema(cls) -> io.Schema:
        orig_cls = getattr(_orig, 'ReferenceLatent')
        orig_schema = orig_cls.define_schema()
        schema = copy.deepcopy(orig_schema)
        base_node_id = schema.node_id or 'ReferenceLatent'
        base_display = schema.display_name or 'ReferenceLatent'
