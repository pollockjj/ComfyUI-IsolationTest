...
class PerpNeg_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='PerpNeg_ISO', display_name='Perp-Neg (DEPRECATED by PerpNegGuider)_ISO', category='_for_testing', inputs=[io.Model.Input('model'), io.Conditioning.Input('empty_conditioning'), io.Float.Input('neg_scale', default=1.0, min=0.0, max=100.0, step=0.01)], outputs=[io.Model.Output()], is_experimental=True, is_deprecated=True)
...
class Guider_PerpNeg(comfy.samplers.CFGGuider):
    ...
    def predict_noise(self, x, timestep, model_options={}, seed=None):
        ...
        for fn in model_options.get('sampler_pre_cfg_function', []):
            args = {'conds': conds, 'conds_out': out, 'cond_scale': self.cfg, 'timestep': timestep, 'input': x, 'sigma': timestep, 'model': self.inner_model, 'model_options': model_options}
            out = fn(args)
        ...
        for fn in model_options.get('sampler_post_cfg_function', []):
            args = {'denoised': cfg_result, 'cond': positive_cond, 'uncond': negative_cond, 'cond_scale': self.cfg, 'model': self.inner_model, 'uncond_denoised': noise_pred_neg, 'cond_denoised': noise_pred_pos, 'sigma': timestep, 'model_options': model_options, 'input': x, 'empty_cond': empty_cond, 'empty_cond_denoised': noise_pred_empty}
            cfg_result = fn(args)
        return cfg_result

class PerpNegGuider_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='PerpNegGuider_ISO', category='_for_testing', inputs=[io.Model.Input('model'), io.Conditioning.Input('positive'), io.Conditioning.Input('negative'), io.Conditioning.Input('empty_conditioning'), io.Float.Input('cfg', default=8.0, min=0.0, max=100.0, step=0.1, round=0.01), io.Float.Input('neg_scale', default=1.0, min=0.0, max=100.0, step=0.01)], outputs=[io.Guider.Output()], is_experimental=True)
...
class PerpNegExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [PerpNeg_ISO, PerpNegGuider_ISO]

async def comfy_entrypoint() -> PerpNegExtension_ISO:
    return PerpNegExtension_ISO()