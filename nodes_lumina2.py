...
class RenormCFG_ISO(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(node_id='RenormCFG_ISO', category='advanced/model', inputs=[io.Model.Input('model'), io.Float.Input('cfg_trunc', default=100, min=0.0, max=100.0, step=0.01), io.Float.Input('renorm_cfg', default=1.0, min=0.0, max=100.0, step=0.01)], outputs=[io.Model.Output()])
...
class CLIPTextEncodeLumina2_ISO(io.ComfyNode):
    SYSTEM_PROMPT = {'superior': 'You are an assistant designed to generate superior images with the superior degree of image-text alignment based on textual prompts or user prompts.', 'alignment': 'You are an assistant designed to generate high-quality images with the highest degree of image-text alignment based on textual prompts.'}
    SYSTEM_PROMPT_TIP = 'Lumina2 provide two types of system prompts:Superior: You are an assistant designed to generate superior images with the superior degree of image-text alignment based on textual prompts or user prompts. Alignment: You are an assistant designed to generate high-quality images with the highest degree of image-text alignment based on textual prompts.'

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='CLIPTextEncodeLumina2_ISO', display_name='CLIP Text Encode for Lumina2_ISO', category='conditioning', description='Encodes a system prompt and a user prompt using a CLIP model into an embedding that can be used to guide the diffusion model towards generating specific images.', inputs=[io.Combo.Input('system_prompt', options=list(cls.SYSTEM_PROMPT.keys()), tooltip=cls.SYSTEM_PROMPT_TIP), io.String.Input('user_prompt', multiline=True, dynamic_prompts=True, tooltip='The text to be encoded.'), io.Clip.Input('clip', tooltip='The CLIP model used for encoding the text.')], outputs=[io.Conditioning.Output(tooltip='A conditioning containing the embedded text used to guide the diffusion model.')])
...
class Lumina2Extension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [CLIPTextEncodeLumina2_ISO, RenormCFG_ISO]

async def comfy_entrypoint() -> Lumina2Extension_ISO:
    return Lumina2Extension_ISO()