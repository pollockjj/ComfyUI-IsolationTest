...
class BasicScheduler_ISO(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(node_id='BasicScheduler_ISO', category='sampling/custom_sampling/schedulers', inputs=[io.Model.Input('model'), io.Combo.Input('scheduler', options=comfy.samplers.SCHEDULER_NAMES), io.Int.Input('steps', default=20, min=1, max=10000), io.Float.Input('denoise', default=1.0, min=0.0, max=1.0, step=0.01)], outputs=[io.Sigmas.Output()])
...
class SDTurboScheduler_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SDTurboScheduler_ISO', category='sampling/custom_sampling/schedulers', inputs=[io.Model.Input('model'), io.Int.Input('steps', default=1, min=1, max=10), io.Float.Input('denoise', default=1.0, min=0, max=1.0, step=0.01)], outputs=[io.Sigmas.Output()])
...
class SplitSigmas_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SplitSigmas_ISO', category='sampling/custom_sampling/sigmas', inputs=[io.Sigmas.Input('sigmas'), io.Int.Input('step', default=0, min=0, max=10000)], outputs=[io.Sigmas.Output(display_name='high_sigmas'), io.Sigmas.Output(display_name='low_sigmas')])
...
class SplitSigmasDenoise_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SplitSigmasDenoise_ISO', category='sampling/custom_sampling/sigmas', inputs=[io.Sigmas.Input('sigmas'), io.Float.Input('denoise', default=1.0, min=0.0, max=1.0, step=0.01)], outputs=[io.Sigmas.Output(display_name='high_sigmas'), io.Sigmas.Output(display_name='low_sigmas')])
...
class FlipSigmas_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='FlipSigmas_ISO', category='sampling/custom_sampling/sigmas', inputs=[io.Sigmas.Input('sigmas')], outputs=[io.Sigmas.Output()])
...
class SetFirstSigma_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SetFirstSigma_ISO', category='sampling/custom_sampling/sigmas', inputs=[io.Sigmas.Input('sigmas'), io.Float.Input('sigma', default=136.0, min=0.0, max=20000.0, step=0.001, round=False)], outputs=[io.Sigmas.Output()])
...
class ExtendIntermediateSigmas_ISO(io.ComfyNode):
    ...
    def execute(cls, sigmas: torch.Tensor, steps: int, start_at_sigma: float, end_at_sigma: float, spacing: str) -> io.NodeOutput:
        ...
        interpolator = {'linear': lambda x: x, 'cosine': lambda x: torch.sin(x * math.pi / 2), 'sine': lambda x: 1 - torch.cos(x * math.pi / 2)}[spacing]
...
class SamplingPercentToSigma_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplingPercentToSigma_ISO', category='sampling/custom_sampling/sigmas', inputs=[io.Model.Input('model'), io.Float.Input('sampling_percent', default=0.0, min=0.0, max=1.0, step=0.0001), io.Boolean.Input('return_actual_sigma', default=False, tooltip='Return the actual sigma value instead of the value used for interval checks.\nThis only affects results at 0.0 and 1.0.')], outputs=[io.Float.Output(display_name='sigma_value')])
...
class KSamplerSelect_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='KSamplerSelect_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Combo.Input('sampler_name', options=comfy.samplers.SAMPLER_NAMES)], outputs=[io.Sampler.Output()])
...
class SamplerDPMPP_3M_SDE_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerDPMPP_3M_SDE_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Float.Input('eta', default=1.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('s_noise', default=1.0, min=0.0, max=100.0, step=0.01, round=False), io.Combo.Input('noise_device', options=['gpu', 'cpu'])], outputs=[io.Sampler.Output()])
...
class SamplerDPMPP_2M_SDE_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerDPMPP_2M_SDE_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Combo.Input('solver_type', options=['midpoint', 'heun']), io.Float.Input('eta', default=1.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('s_noise', default=1.0, min=0.0, max=100.0, step=0.01, round=False), io.Combo.Input('noise_device', options=['gpu', 'cpu'])], outputs=[io.Sampler.Output()])
...
class SamplerDPMPP_SDE_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerDPMPP_SDE_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Float.Input('eta', default=1.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('s_noise', default=1.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('r', default=0.5, min=0.0, max=100.0, step=0.01, round=False), io.Combo.Input('noise_device', options=['gpu', 'cpu'])], outputs=[io.Sampler.Output()])
...
class SamplerDPMPP_2S_Ancestral_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerDPMPP_2S_Ancestral_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Float.Input('eta', default=1.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('s_noise', default=1.0, min=0.0, max=100.0, step=0.01, round=False)], outputs=[io.Sampler.Output()])
...
class SamplerEulerAncestral_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerEulerAncestral_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Float.Input('eta', default=1.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('s_noise', default=1.0, min=0.0, max=100.0, step=0.01, round=False)], outputs=[io.Sampler.Output()])
...
class SamplerEulerAncestralCFGPP_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerEulerAncestralCFGPP_ISO', display_name='SamplerEulerAncestralCFG++_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Float.Input('eta', default=1.0, min=0.0, max=1.0, step=0.01, round=False), io.Float.Input('s_noise', default=1.0, min=0.0, max=10.0, step=0.01, round=False)], outputs=[io.Sampler.Output()])
...
class SamplerLMS_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerLMS_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Int.Input('order', default=4, min=1, max=100)], outputs=[io.Sampler.Output()])
...
class SamplerDPMAdaptative_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerDPMAdaptative_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Int.Input('order', default=3, min=2, max=3), io.Float.Input('rtol', default=0.05, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('atol', default=0.0078, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('h_init', default=0.05, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('pcoeff', default=0.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('icoeff', default=1.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('dcoeff', default=0.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('accept_safety', default=0.81, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('eta', default=0.0, min=0.0, max=100.0, step=0.01, round=False), io.Float.Input('s_noise', default=1.0, min=0.0, max=100.0, step=0.01, round=False)], outputs=[io.Sampler.Output()])
...
class SamplerER_SDE_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerER_SDE_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Combo.Input('solver_type', options=['ER-SDE', 'Reverse-time SDE', 'ODE']), io.Int.Input('max_stage', default=3, min=1, max=3), io.Float.Input('eta', default=1.0, min=0.0, max=100.0, step=0.01, round=False, tooltip="Stochastic strength of reverse-time SDE.\nWhen eta=0, it reduces to deterministic ODE. This setting doesn't apply to ER-SDE solver type."), io.Float.Input('s_noise', default=1.0, min=0.0, max=100.0, step=0.01, round=False)], outputs=[io.Sampler.Output()])
...
class SamplerSASolver_ISO(io.ComfyNode):
    ...
    def execute(cls, model, eta, sde_start_percent, sde_end_percent, s_noise, predictor_order, corrector_order, use_pece, simple_order_2) -> io.NodeOutput:
        ...
        sampler = comfy.samplers.ksampler(sampler_name, {'tau_func': tau_func, 's_noise': s_noise, 'predictor_order': predictor_order, 'corrector_order': corrector_order, 'use_pece': use_pece, 'simple_order_2': simple_order_2})
        return io.NodeOutput(sampler)
    get_sampler = execute
...
class BasicGuider_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='BasicGuider_ISO', category='sampling/custom_sampling/guiders', inputs=[io.Model.Input('model'), io.Conditioning.Input('conditioning')], outputs=[io.Guider.Output()])
...
class CFGGuider_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='CFGGuider_ISO', category='sampling/custom_sampling/guiders', inputs=[io.Model.Input('model'), io.Conditioning.Input('positive'), io.Conditioning.Input('negative'), io.Float.Input('cfg', default=8.0, min=0.0, max=100.0, step=0.1, round=0.01)], outputs=[io.Guider.Output()])
...
class DualCFGGuider_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='DualCFGGuider_ISO', category='sampling/custom_sampling/guiders', inputs=[io.Model.Input('model'), io.Conditioning.Input('cond1'), io.Conditioning.Input('cond2'), io.Conditioning.Input('negative'), io.Float.Input('cfg_conds', default=8.0, min=0.0, max=100.0, step=0.1, round=0.01), io.Float.Input('cfg_cond2_negative', default=8.0, min=0.0, max=100.0, step=0.1, round=0.01), io.Combo.Input('style', options=['regular', 'nested'])], outputs=[io.Guider.Output()])
...
class DisableNoise_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='DisableNoise_ISO', category='sampling/custom_sampling/noise', inputs=[], outputs=[io.Noise.Output()])
...
class RandomNoise_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='RandomNoise_ISO', category='sampling/custom_sampling/noise', inputs=[io.Int.Input('noise_seed', default=0, min=0, max=18446744073709551615, control_after_generate=True)], outputs=[io.Noise.Output()])
...
class SamplerCustomAdvanced_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='SamplerCustomAdvanced_ISO', category='sampling/custom_sampling', inputs=[io.Noise.Input('noise'), io.Guider.Input('guider'), io.Sampler.Input('sampler'), io.Sigmas.Input('sigmas'), io.Latent.Input('latent_image')], outputs=[io.Latent.Output(display_name='output'), io.Latent.Output(display_name='denoised_output')])
...
class AddNoise_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='AddNoise_ISO', category='_for_testing/custom_sampling/noise', is_experimental=True, inputs=[io.Model.Input('model'), io.Noise.Input('noise'), io.Sigmas.Input('sigmas'), io.Latent.Input('latent_image')], outputs=[io.Latent.Output()])
