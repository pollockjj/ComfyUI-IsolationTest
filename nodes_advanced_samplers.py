...
class SamplerLCMUpscale_ISO(io.ComfyNode):
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(node_id='SamplerLCMUpscale_ISO', category='sampling/custom_sampling/samplers', inputs=[io.Float.Input('scale_ratio', default=1.0, min=0.1, max=20.0, step=0.01), io.Int.Input('scale_steps', default=-1, min=-1, max=1000, step=1), io.Combo.Input('upscale_method', options=cls.UPSCALE_METHODS)], outputs=[io.Sampler.Output()])
...
class SamplerEulerCFGpp_ISO(io.ComfyNode):
    ...
    @classmethod
    def define_schema(cls) -> io.Schema:
        return io.Schema(node_id='SamplerEulerCFGpp_ISO', display_name='SamplerEulerCFG++_ISO', category='_for_testing', inputs=[io.Combo.Input('version', options=['regular', 'alternative'])], outputs=[io.Sampler.Output()], is_experimental=True)
...
class AdvancedSamplersExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [SamplerLCMUpscale_ISO, SamplerEulerCFGpp_ISO]

async def comfy_entrypoint() -> AdvancedSamplersExtension_ISO:
    return AdvancedSamplersExtension_ISO()