...
class CLIPTextEncodeSDXLRefiner_ISO(io.ComfyNode):

    @classmethod
    def define_schema(cls):
        return io.Schema(node_id='CLIPTextEncodeSDXLRefiner_ISO', category='advanced/conditioning', inputs=[io.Float.Input('ascore', default=6.0, min=0.0, max=1000.0, step=0.01), io.Int.Input('width', default=1024, min=0, max=nodes.MAX_RESOLUTION), io.Int.Input('height', default=1024, min=0, max=nodes.MAX_RESOLUTION), io.String.Input('text', multiline=True, dynamic_prompts=True), io.Clip.Input('clip')], outputs=[io.Conditioning.Output()])
...
class ClipSdxlExtension_ISO(ComfyExtension):

    @override
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [CLIPTextEncodeSDXLRefiner_ISO, CLIPTextEncodeSDXL_ISO]

async def comfy_entrypoint() -> ClipSdxlExtension_ISO:
    return ClipSdxlExtension_ISO()